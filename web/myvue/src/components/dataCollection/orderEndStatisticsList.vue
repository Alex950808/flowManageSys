<template>
  <div class="orderEndStatisticsList">
      <el-row>
                ...
                <el-col :span="22" :offset="1">
                    ...
                    <el-row class="lineHeightForty fontWeight MB_ten border B_R fontLift">
                        <div class="lineHeightForty">
                            ...
                            <el-col :span="4"><span><i class="NotStyleForI F_S_Sixteen">实采逻辑毛利</i> ：<i class="NotStyleForI F_S_twenty">{{titleInfo.sub_profit_rate}}%</i></span></el-col>
                        </div>
                    </el-row>
                    <!-- <div>  
                    </div> -->
                    <div class="border B_R MB_twenty PR" v-for="(item,index) in tableData"> 
                        <el-row class="Lavender">
                            ...
                            <el-col :span="2" class="fontRight MT_Thirty">
                                <span @click="viewDetails(item.sub_order_sn,index)" class="notBgButton"><i class="NotStyleForI">展开</i><i class="el-icon-arrow-down" title="点击展示更多"></i></span>
                            </el-col>
                        </el-row>
                        <el-row class="fontCenter Lavender">
                            ...
                            <el-col :span="5">
                                <span class="lineHeightThirty"><i class="NotStyleForI F_Z_N">目标逻辑毛利</i><i class="NotStyleForI ML_twenty F_S_twenty fontWeight d_I_B widthOneHundred">{{item.target_rate}}%</i></span><br>
                                <span class="lineHeightThirty">
                                    <i class="NotStyleForI F_Z_N">报价逻辑毛利</i>
                                    <i v-if="item.sub_quote_rate>item.target_rate" class="NotStyleForI blueFont ML_twenty F_S_twenty fontWeight d_I_B widthOneHundred ">{{item.sub_quote_rate}}%
                                        <i class="iconfont">&#xe6b4;</i>
                                    </i>
                                    <i v-if="item.sub_quote_rate<item.target_rate" class="NotStyleForI redFont ML_twenty F_S_twenty fontWeight d_I_B widthOneHundred ">{{item.sub_quote_rate}}%
                                        <i class="iconfont">&#xe6b4;</i>
                                    </i>
                                </span><br>
                                <span class="lineHeightThirty">
                                    <i class="NotStyleForI F_Z_N">实采逻辑毛利</i>
                                    <i v-if="item.sub_profit_rate>item.target_rate" class="NotStyleForI blueFont ML_twenty F_S_twenty fontWeight d_I_B widthOneHundred">{{item.sub_profit_rate}}%
                                        <i class="iconfont">&#xe644;</i>
                                    </i>
                                    <i v-if="item.sub_profit_rate<item.target_rate" class="NotStyleForI redFont ML_twenty F_S_twenty fontWeight d_I_B widthOneHundred">{{item.sub_profit_rate}}%
                                        <i class="iconfont">&#xe644;</i>
                                    </i>
                                </span>
                            </el-col>
                        ...
                        <div class="O_F_A w_ratio PR" v-if="index==indexnum">
                            ...
                            <table class="fontCenter tabletitle" :style="tableWidth">
                                ...
                                <tr v-for="orderInfo in tableDataTwo.sub_order_goods_list">
                                    ...
                                    <td width="100px;" v-if="selectTitle('exw折扣')">{{orderInfo.exw_discount}}</td>
...
export default {
  ...
  data(){
    return{
      ...
      tableData:[],
      tableDataTwo:[],
      tableTitle:['商品名称','商品规格码','商家编码','美金原价','总需求量','现货数量','需求量','待采量','分货数量','报价折扣','商品重量','exw折扣','重价率','报价逻辑毛利','实际逻辑毛利'],
      tabletitle:[],
      titleInfo:[],
      purchaseSumDate:'',
      isShow:false,
      ...
      is_caigoubu:'',
      //选择列
      selectStr:['商品名称','商品规格码','商家编码','美金原价','总需求量','现货数量','需求量','待采量','分货数量','报价折扣','商品重量','exw折扣','重价率','报价逻辑毛利','实际逻辑毛利'],
      //搜索订单
      orderType:'',
      sale_user_info:'',
      sale_user_id:'',
  ...
  methods:{
    getDataList(){
        ...
        axios.get(vm.url+vm.$orderCurrentStatisticsListURL+"?start_time="+vm.startTime+"&end_time="+vm.endTime+"&query_sn="+vm.subOrderSn+"&is_combine="+vm.is_combine+
            "&sale_user_id="+vm.sale_user_id+"&order_type="+vm.orderType+"&page="+vm.page+"&page_size="+vm.pagesize+"&demand_sort_status=2"+
            "&sum_date_cat=1"+"&batch_type="+vm.batch_type+"&sum_demand_sn="+vm.sumDemandSn,
            {
    ...
    viewDetails(sub_order_sn,index){
        ...
        vm.tableDataTwo=[];
        vm.tabletitle=[];
        vm.selectStr.splice(15);
        let content_text_height=$(event.target).parent().parent().parent().parent().height();
        if(content_text_height<=235){
            let load = Loading.service({fullscreen: true, text: '拼命加载中....'});
            axios.get(vm.url+vm.$subOrderStatisticsListURL+"?sub_order_sn="+sub_order_sn+"&demand_sort_status=2"+"&batch_type="+vm.batch_type,
                {
                    headers:vm.headersStr,
                },
            ).then(function(res){
