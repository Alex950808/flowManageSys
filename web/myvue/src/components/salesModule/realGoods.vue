<template>
  <div class="realGoods_b">
    <!-- 实采商品分配页面 -->
    <!-- <el-row :gutter="20"> -->
        <el-col :span="24" style="background-color: #fff;">
            <el-row>
                <el-col :span="22" :offset="1">
                    <div class="realGoods bgDiv">
                        <div class="title">
                            <span><span class="coarseLine"></span>&nbsp;&nbsp;&nbsp;实采商品分配</span><input class="inputStyle" placeholder="请输入搜索关键字" type="text" v-model="search" @keyup.enter="searchFrame"/><i class="el-icon-search search"></i>
                        </div>
                        <div class="content" v-for="item in BatchData">
                            <div class="content_text">
                                ...
                                <span class="status"  @click="goSummaryPage(item.title_info.purchase_sn)"><i class="el-icon-goods"></i>&nbsp&nbsp查看数据汇总</span>
                            </div>
                            <el-row>
                                <el-col :span="12">
    ...
    methods: {
      //获取实采商品分配
        getBatchData(){
            ...
            axios.get(vm.url+vm.$getGoodsActAllotListURL+"?start_page="+vm.page+"&page_size="+vm.pagesize+"&query_sn="+vm.search,
                {
                     headers:{
      ...
      goSummaryPage(purchase_sn){
          this.$router.push('/goodsSeeData?purchase_sn='+purchase_sn+'&isSummary=isSummary');
      },
      goDetails(purchase_sn,real_purchase_sn){
            let vm=this;
            let headersToken=sessionStorage.getItem("token");
            axios.get(vm.url+vm.$goodsRealAllotURL+"?purchase_sn="+purchase_sn+"&real_purchase_sn="+real_purchase_sn,
                {
                    headers:{
                        'Authorization': 'Bearer ' + headersToken,
                        'Accept': 'application/vnd.jmsapi.v1+json',
                ...
                vm.$router.push('/goodsSeeData?purchase_sn='+purchase_sn+'&real_purchase_sn='+real_purchase_sn+'&isDetails=isDetails');
            }).catch(function (error) {
        ...
        //   this.$router.push('/goodsSeeData?purchase_sn='+purchase_sn+'&real_purchase_sn='+real_purchase_sn+'&isDetails=isDetails');
      },
        //搜索框
      searchFrame(){
          let vm=this;
