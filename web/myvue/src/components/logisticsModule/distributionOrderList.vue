...
export default {
  ...
  data(){
    return{
      ...
      tableContent:'',
      TCTitle:['配货单单号','订单总价','订单状态','sku数','查看详情'],
  ...
  methods:{
    getDataList(){
        let vm = this;
        axios.get(vm.url+vm.$distributionOrderListURL+"?page="+vm.page+"&pagesize="+vm.pagesize,
            {
    ...
    ViewDetail(sn){
        let vm = this;
        vm.$router.push('/distributionOrderDetail?deliver_order_sn='+sn);
    },
    //分页
