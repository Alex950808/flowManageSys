<?php
//created by zhangdong on the 2018.10.25
//url:https://github.com/huyanping/Zebra-PHP-ArrayGroupBy
...
class ArrayGroupBy
{
    ...
    public function groupByField($group_field)
    {
        ...
        $grouped = call_user_func_array('self::groupByFieldDeep', $group_by_field_params);
    ...
    {
        if (!is_array($arr)) {
            $message = 'ArrayGroupBy::groupByFieldDeep(): The first argument should be an array';
            return $message;
        }
        if (!is_string($key) && !is_int($key) && !is_float($key)) {
            $message = 'ArrayGroupBy::groupByFieldDeep(): The key should be a string or an integer';
            return $message;
        }
        ...
        if (func_num_args() > 3) {
            ...
            foreach ($grouped as $key => $value) {
                ...
                $grouped[$key] = call_user_func_array("ArrayGroupBy::groupByFieldDeep", $params);
            }
        }
        return $grouped;
    }
    ...
    public static function getDeepestArray($array)
    {
        ...
        foreach ($array as $arr) {
            if (!is_array($arr)) {
                ...
                $sub_result = call_user_func("ArrayGroupBy::getDeepestArray", $arr);
    ...
    public static function arrayColumn($array, $column_key, $index_key = null)
    {
        if (!is_array($array) && !($array instanceof \ArrayAccess))
            $message = "Argument 1 passed to ArrayGroupBy::::arrayColumn() must be of the type array";
            return $message;
